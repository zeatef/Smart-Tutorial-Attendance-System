...
{
	public partial class TodayInstructor : ContentPage
	{
		...
		public TodayInstructor (SQLDatabase database, Instructor ins)
		{
			this.user = ins;
			...
			title.Text = _database.GetInstructorName (user.tid);
...
//			weekday = "Tuesday";
			...
			if (_database.InstructorTakesToday (_database.GetInstructorName (user.tid), weekday)) {
				...
				_data.ItemTemplate = new DataTemplate (typeof(InstructorCustomCell));
				_data.ItemTapped += async (sender, e) => {
					enroll_view item = (enroll_view)e.Item;
					await Navigation.PushAsync (new GUC_Attendance.InstructorTutorialPage (_database, item, w_no, datenow));
				};
				stack.Children.Add (mycourses);
				stack.Children.Add (_data);
			} else {
				Label label = new Label {
					Text = "You don't have any tutorials today.",
